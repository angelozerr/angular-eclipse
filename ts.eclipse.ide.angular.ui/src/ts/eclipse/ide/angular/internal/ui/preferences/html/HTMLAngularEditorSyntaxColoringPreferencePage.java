 ...
 */
package ts.eclipse.ide.angular.internal.ui.preferences.html;
...
import ts.eclipse.ide.angular.internal.ui.AngularUIMessages;
import ts.eclipse.ide.angular.internal.ui.Trace;
import ts.eclipse.ide.angular.internal.ui.style.IStyleConstantsForAngular;
import ts.eclipse.ide.angular.internal.ui.style.SemanticHighlightingManager;
 ...
 */
public final class HTMLAngularEditorSyntaxColoringPreferencePage extends AbstractSyntaxColoringPage
		implements IWorkbenchPreferencePage {

	private static final String EXAMPLE_TEXT_FILE_NAME = HTMLAngularEditorSyntaxColoringPreferencePage.class.getName()
			+ ".xml";
	...
	private IDOMModel getDomModel() throws IOException {
		...
		model = (IDOMModel) StructuredModelManager.getModelManager().getModelForRead(EXAMPLE_TEXT_FILE_NAME,
				inputStream, null);
		return model;
	}
	...
	public void init(IWorkbench workbench) {
		...
		IPreferenceStore store = new MyChainedPreferenceStore(
				new IPreferenceStore[] { getPreferenceStore(), HTMLUIPlugin.getDefault().getPreferenceStore() });
	...
	private void initStyleToDescriptionMap() {
		...
		fStyleToDescriptionMap.put(IStyleConstantsXML.ENTITY_REFERENCE, XMLUIMessages.Entity_Reference_UI_); // $NON-NLS-1$
																												// =
																												// "Entity
																												// References"
		...
		fStyleToDescriptionMap.put(IStyleConstantsForAngular.ANGULAR_EXPRESSION_BORDER,
				AngularUIMessages.Angular_expression_border);
		fStyleToDescriptionMap.put(IStyleConstantsForAngular.ANGULAR_EXPRESSION,
				AngularUIMessages.Angular_expression);
		fStyleToDescriptionMap.put(IStyleConstantsForAngular.ANGULAR_DIRECTIVE_NAME,
				AngularUIMessages.Angular_directive_name);
	}
