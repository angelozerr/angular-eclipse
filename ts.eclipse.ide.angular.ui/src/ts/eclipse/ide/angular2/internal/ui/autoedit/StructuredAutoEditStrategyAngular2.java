...
/**
 * {@link IAutoEditStrategy} implementation to close Angular2 expression: in
...
public class StructuredAutoEditStrategyAngular2 extends StructuredAutoEditStrategy {
	...
	private void smartInsertCloseEndEL(DocumentCommand command, IDocument document, IStructuredModel model) {
		try {
			if (isPreferenceEnabled(Angular2UIPreferenceNames.TYPING_COMPLETE_END_EL) && command.text.equals("{") //$NON-NLS-1$
	...
	private boolean isPreferenceEnabled(String key) {
		return (key != null && Angular2UIPlugin.getDefault().getPreferenceStore().getBoolean(key));
	}
